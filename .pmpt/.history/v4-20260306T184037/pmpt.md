...
- [x] NetTracker — per-process network tracking (Priority 2)
...
- Implemented `collectors/net_tracker.py`: psutil.net_connections polling, reverse DNS caching, allowlist matching.
...
- Added `_evaluate_net_event()` to AlertEngine. Refactored `evaluate_security_event()` to dispatch by source.
- Integrated NetTracker into daemon main loop (polling-based, not threaded).
- Added `security.net_tracker` config section with 12 default allowlist domains.
- 23 new tests (allowlist, AI detection, DNS, polling, alert evaluation).
- Test result: `96 passed` (73 + 23 new).
