...
### v1 — Initial Setup
- Project initialized with pmpt for development tracking
- Sentinel v0.1.0 was already released as an AI Session Guardian for macOS with system monitoring (CPU, battery, thermal, memory, disk, network) and ntfy.sh phone notifications
- Established baseline codebase with MacOSCollector, AlertEngine, and NtfyNotifier pipeline

### v2 — Security Posture Watch
- Added security posture monitoring for macOS Firewall, Gatekeeper, and FileVault using native command probes (socketfilterfw, spctl, fdesetup)
- Extended SystemMetrics with posture signals and added security_posture alert generation with cooldown handling
- Fixed two baseline regressions in load_config YAML handling and cooldown timestamp behavior
- All 46 tests passing

### v3 — AI Security Layer Foundation
- Chose layered architecture (Option 3) to add AI Security Layer on top of existing system monitor
- Created comprehensive spec document covering FSWatcher, NetTracker, and AgentLogParser designs
- Refactored monolithic core.py into 5 modules (models.py, collectors/system.py, engine.py, notifier.py) with re-export backward compatibility — all 46 tests passed unchanged
- Added SecurityEvent dataclass as the common data model for all security collectors

### v4 — FSWatcher Implementation
- Implemented FSWatcher using watchdog Observer with lsof-based AI process attribution
- Detects sensitive path access (~/.ssh, .env), executable file creation, bulk file changes, and AI process file activity
- Added evaluate_security_event() to AlertEngine with source-based dispatch and cooldown
- Integrated into Sentinel daemon via shared queue.Queue with drain loop
- 27 new tests, total 73 passed

### v5 — NetTracker + AgentLogParser + Docs Overhaul
- Implemented NetTracker with psutil.net_connections polling, reverse DNS caching, fnmatch allowlist matching, and 5-min TTL deduplication
- Implemented AgentLogParser with Claude Code JSONL tail-f parsing, 14 pre-compiled HIGH_RISK_PATTERNS, and tool_use content block analysis
- Major documentation overhaul: rewrote pmpt.md with 10 detailed Architecture Decisions and pmpt.ai.md as comprehensive AI context document
- 54 new tests (23 NetTracker + 31 AgentLogParser), total 127 passed

### v6 — JSONL Event Logging + Integration Tests
- Implemented EventLogger for append-only daily JSONL event logging, recording all SecurityEvents to ~/.local/share/sentinel/events/ for Phase 2 team dashboard preparation
- Added end-to-end integration tests verifying full event flow from collectors through queue drain to JSONL output and alert generation
- 11 new tests (6 EventLogger unit + 5 integration), total 138 passed

### v7 — Multi-channel Notifications + Critical-only Alerting
- Redesigned notification system from single NtfyNotifier to multi-channel NotificationManager supporting macOS native (default, no setup), ntfy.sh (opt-in), and Slack webhook (opt-in)
- Adopted "value means enabled" pattern — setting a config value auto-enables the channel
- Implemented critical-only alerting principle: only critical events push notifications, warning/info logged to JSONL only, reducing alert fatigue
- Updated pmpt.ai.md with full Architecture Decisions (AD-11, AD-12) and current project state
- 23 notifier tests, total 155 passed

### v8 — 2026-03-06
- Backfilled all 7 snapshot versions with detailed progress notes for publish readiness
- Each version now has a descriptive summary covering what was built, why, and key technical decisions — from initial setup through security posture monitoring, AI security layer foundation, FSWatcher, NetTracker, AgentLogParser, JSONL event logging, and multi-channel notifications with critical-only alerting
