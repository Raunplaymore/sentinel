...
세 가지 설치 경로 중 워크플로우에 맞는 것을 고르세요. 어느 쪽을 골라도 마지막에는 `sentinel` CLI가 `PATH`에 노출됩니다.

### 옵션 1 — 권장: pipx (단일 머신, 격리된 venv)

`pipx`가 배포 wheel을 자체 venv에 설치하고 `sentinel` 실행 파일을 전역에 노출합니다. repo clone, venv 수동 설정 모두 불필요.
...
트레이드오프: `install.sh`의 launchd 자동 시작 단계는 빠집니다. 부팅 시 데몬이 자동으로 뜨길 원하면 아래 "로그인 시 자동 시작" 가이드를 참고.

### 옵션 2 — git clone + install.sh (스크립트 셋업 + launchd 자동 시작)
...
격리된 venv를 만들고, 의존성을 설치하고, launchd 서비스를 등록(로그인 시 자동 시작)하고, `sentinel` 별칭을 셸에 추가합니다. 소스 수정/기여를 원하는 경우 가장 추천. 터미널을 재시작한 후:
...
이미 본인이 venv를 관리하고 있고 가장 가벼운 설치를 원할 때. 로그인 자동 시작은 LaunchAgent plist를 직접 작성해야 합니다.

설치하면 macOS 기본 알림이 바로 동작합니다. 별도 앱 설치 불필요.

### Claude Code 훅 통합 (1회 설정)

Claude Code를 사용 중이라면 pre-tool-use 훅을 설치해 모든 Bash / Write / WebFetch / MCP 호출 **이전에** Sentinel이 검사하도록 만드세요.

```bash
sentinel hooks install      # 1회
sentinel hooks status       # 등록 확인
sentinel hooks uninstall    # 롤백
```

훅을 설치하지 않아도 Sentinel은 로그 tailing으로 이벤트를 감지하지만, 훅을 설치하면 감지가 **동기적**으로 바뀝니다 — Claude Code가 위험한 도구 호출 직전에 사용자에게 묻거나 차단합니다. `hooks install` 후에는 Claude Code를 재시작해야 적용됩니다.
